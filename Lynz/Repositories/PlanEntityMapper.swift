...
class PlanEntityMapper {
    ...
    // MARK: - EntityMapperProtocol Implementation
    ...
    func createFetchRequest<T: Codable & Identifiable>(for type: T.Type) -> NSFetchRequest<NSManagedObject> {
        if type == Plan.self {
            let request = coreDataService.createFetchRequest(for: PlanEntity.self)
    ...
    private func findPlanEntity(for plan: Plan) throws -> PlanEntity {
        let fetchRequest = coreDataService.createFetchRequest(for: PlanEntity.self)
...
//            guard let name = taskEntity.name else { return nil }
